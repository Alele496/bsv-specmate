...
>
> ⚠️ **议会 S02E03 裁定（2026-07-14）已推翻本文档的部分决策。** 当前架构：**MCP 为 Agent 唯一正式通道**，CLI 降级为人类调试辅助。以下内容保留作为历史记录和设计意图参考，当前实际架构以 `project-memory.md` 为准。

## 定位

specmate 是 **BSV 编码全周期平台**——不是"Agent 的可选顾问"，不是"bsc 的包装器"，而是 Agent 开发 BSV 的唯一检查/编译入口。

Agent 不需要知道 bsc 怎么用——它只需要知道 specmate_scan（检查）、specmate_check（静态扫描）和 specmate_diagnose（编译错误诊断）。bsc 由 Agent 直接调用 shell 运行。
...
| L1 | CLI（npx specmate scan/check） | Bash 调用 | 人类调试辅助。⚠️ S02E03 裁定 MCP 为 Agent 唯一通道，CLI 不用于 Agent |
| L2 | MCP（8 个工具：scan/check/guide/capture/resolve/analyze/diagnose/diff） | 交互式调用 | ⭐ 主通道（S02E03 裁定）。Agent 唯一正式通道
...
1. 理解任务 → 写 .bsv 文件
2. 调 `specmate_scan` 了解设计陷阱和约束
3. 写代码后调 `specmate_check` 静态检查
4. 修复 check 报告的所有问题
5. 重新 check 直到通过
6. 调 `bsc` 编译生成 Verilog
...
- ⚠️ CLI 仅供人类开发者调试。Agent 通过 MCP 工具调用。
...
| Agent 交互主通道 | MCP（8 个工具）| S02E03 裁定 MCP 为 Agent 唯一正式通道，标准 BSV 开发工作流 |

## 明确不做的事

- ❌ HTTP server / localhost 端口 —— 不做。Agent 不需要手动启动服务
- ❌ daemon 常驻进程 —— 不做。不增加运维负担
- ❌ SPP 推送协议 —— 休眠（代码保留为 no-op，基础设施不活跃）
...
- `specmate_bench/CLAUDE.md` — bench 实验平台
- `kova/CLAUDE.md` — kova 知识引擎（远期）
