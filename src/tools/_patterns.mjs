...
endrule`,
        ...
        traps: ['G0010: FIFO enq/deq 跨 rule — 用 mkFIFOF 显式分离', 'G0004: 同 rule 内操作多个 FIFO', '流水线平衡 — 慢 stage 会 backpressure 全链路'],
...
endrule`,
        ...
        traps: ['T0051: SPI 命令字 Bit#(8) 注意位宽', 'T0060: 移位寄存器宽度 vs 接收数据宽度', 'SCK 分频用 Bit#(n) 而非 Integer'],
        cross: ['stdlib'],
    },
...
endrule`,
        ...
        traps: ['波特率分频用 Bit#(n) 而非 Integer', 'UART 帧格式 start + 8bit + stop = 10 bits', 'RX oversampling 用 16x 波特率采样'],
        cross: ['stdlib'],
    },
...
endrule`,
        ...
        traps: ['BSV-PORTS: AXI4 接口 port 名与 BSV method 名不一致', 'tkeep = replicate(1) 表示所有 byte lanes 有效', 'G0010: AXI master/slave 方法调用跨 rule'],
