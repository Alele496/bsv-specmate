...
**specmate 就是填这个坑的。** 它不帮 Agent 编译——它在写代码之前先把坑指出来。26 条编码记忆（SQLite 驱动，命中自动 +1，高频排第一）、17 条静态检查规则、30 个领域知识图谱节点、4,570 个官方示例。通过 8 个 MCP 工具嵌进 AI Agent——**`specmate_scan` 是推荐入口**，让 Agent 写 BSV 的时候背后有个 mate 在耳边 bb。
