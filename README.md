...
我们用 RISC-V 外设子系统来了一场对照实验——两个 Agent 同时开发 7 个模块，
一个带 specmate（tapeout 级），一个空手上阵。

结果：🅱️ 带 specmate 的 Agent 少花了 **25% 的编译修复轮数**（9 vs 12），
节省了 **12.6% 的 Token**（149.7K vs 171.3K）。
更妙的是——它被引导着选了更安全的设计风格：标准库 FIFOF 而非手写环形缓冲区，
Bit#(1) 而非 Bool 做控制信号，主动标注调度关系。

想看逐回合翻车现场和分析 → **[📖 完整争霸赛报告](docs/SHOWDOWN.md)**
