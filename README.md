...
**specmate 就是填这个坑的。** 它不帮 Agent 编译——它在写代码之前先把坑指出来。26 条编码记忆（SQLite 驱动，命中自动 +1，高频排第一）、19 条静态检查规则、30 个领域知识图谱节点、4,570 个官方示例。通过 8 个 MCP 工具嵌进 AI Agent——**`specmate_scan` 是推荐入口**，让 Agent 写 BSV 的时候背后有个 mate 在耳边 bb。

> specmate 是 **Kova**（Knowledge Vault，领域知识引擎框架）在 BSV 领域的第一只 mate。核心架构 = DKE（Domain Knowledge Engine）+ 编码记忆 + 约束链 + 角色激活。详见 **[Kova →](https://github.com/Alele496/kova)** 和 `docs/archive/collaboration.md`。
...
| **🔍 编译前体检** | 19 条规则扫 .bsv：方法顺序、Bool 运算符误用、SV 保留字冲突、字面量溢出、interface Bool 返回、always_ready 滥用……不调 bsc，纯静态 | `specmate_check` |
...
| **🏥 全量诊断** | 传入 BSC 完整编译输出，一次调用批量诊断所有错误码/警告，逐个匹配知识库输出根因和修复方案 | `specmate_diagnose` |
...
- 🚀 [快速开始](#-快速开始)
- 🛠 [本地开发](#-本地开发)
- 📖 [详细教程 → docs/archive/TUTORIAL.md](./docs/archive/TUTORIAL.md)
...
| 场景 | 推荐模式 | 模板 | 效果 |
|------|---------|------|------|
| **大项目 / 新模块开发** | 🤝 **协作开发** (Supervisor + Developer) | [docs/archive/collaboration.md](docs/archive/collaboration.md) | 最高通过率，编码时间 -47% |
...
├── package.json
├── bin/
│   ├── server.mjs         ← MCP Server 入口，注册 8 个 MCP 工具
...
| 全量诊断 | **`specmate_diagnose`** | `specmate_diagnose({ bsc_output: "<BSC 完整编译输出>" })` |
| 单错诊断 | **`specmate_guide`** | `specmate_guide({ phase: "on_error", input: "<错误信息>" })` |
