...
最初这只是一个 BSV 编译错题本——Agent 写代码老是编译不过，
我手动记下错误码和修复方案，贴在 `docs/errors/` 下让它翻。

后来发现只是记下来不够——Agent 根本不会主动翻文档。
于是加了 SQLite 自动排序（高频排第一）、命中自动 +1、`coding_rules()` 动态约束。

还是不够——Agent 还是不调工具。于是两场实验验证后，
发现关键是**给 Agent 一个审查角色**（Supervisor），把"检查代码质量"
变成它的职责——工具调用从 0 飙到 10+。

最后发现这不再是个"错题本"，而是一个可复用的领域知识引擎架构——**Kova 框架**。
specmate 是 Kova 在 BSV 领域的第一个完整实例。

> 从错题本到知识引擎，更多关于架构设计的讨论 → **[Kova 框架](https://github.com/Alele496/kova)**
