...
**specmate 就是填这个坑的。** 它不帮 Agent 编译——它在写代码之前先把坑指出来。18 条编码记忆（SQLite 驱动，命中自动 +1，高频排第一）、13 篇语言参考、18 条静态检查规则、5 种编码风格、4,570 个官方用例。通过 MCP 协议嵌进 AI Agent，让 Agent 写 BSV 的时候背后有个 mate 在耳边 bb。
...
| **📦 零配置** | `npm install -g` 一行装好，`.mcp.json` 三行配置。Agent 自动发现 MCP 工具 | — |
